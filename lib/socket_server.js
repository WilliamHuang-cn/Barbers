var socketio = require('socket.io');
var queue;
var io;

exports.listen = (server,inqueue) => {
    queue = inqueue;
    var io = socketio(server);
    io.set('log level',1);
    io.sockets.on('connection', (socket) => {

        console.log('Client successfully connect to server via socket.io!');

        socket.on('registerInfo',() => {
            // TODO: complete registerInfo handout

        });

        socket.on('joinQueue',(customerInfo) => {
            console.log(`Customer join queue: ${customerInfo}`);
            queue.addCustomerToQueue({name:customerInfo.name,openid:customerInfo.openid,serviceType:customerInfo.seriveType,tel:customerInfo.tel,sex:customerInfo.sex,remark:customerInfo.remark},(err) => {
                if (err != null) {
                    console.log(err);
                    socket.emit('joinResult',{success:false,msg:err.message});
                }
                else {
                    console.log('Join queue success');
                    socket.emit('joinResult',{success:true,msg:null});
                }
            });
        });

        socket.on('monitorQueue',() => {
            queue.queueInfo((err,customers) => {
                socket.emit('queueInfo',customers);
            });
        });

        socket.on('disconnect',() => {
            console.log('Client successfully disconnected from server!');
        });
    });
};

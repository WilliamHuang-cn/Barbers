...
// TODO: Impelment this in serviceList.js
...
// TODO: Use serviceList to fetch estimatedTime instea
exports.totalEstimatedTime = (customerID,cb) => {
    ...
    if ((customerID == undefined) || (customerID == null)) {
        ...
        for(var i = 0,len = queue.length; i < len; i ++){
            if(queue[i].openid == customerID) break;
            add(queue[i]);
        }
    }
    cb(err,totalETime,totalCustomer);
};
...
exports.haveCustomer(options,cb) {
    var err = null;
    if (!options.hasOwnProperty('openid')) {
        err = new Error('No customer identification found in options');
        cb(err,false);
